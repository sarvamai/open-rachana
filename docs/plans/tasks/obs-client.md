# Plan: Add bounded browser and signed-client diagnostic telemetry
...
Issue: planned slice `obs-client`. Companion reference: PR #100; contributor workflow: PR #99.
...
Instrument approved page/load/error signals and the signed client when its contract exists. Keep session-integrity evidence on its dedicated durable path.
...
planned slice `obs-next`, planned slice `client-contract`
